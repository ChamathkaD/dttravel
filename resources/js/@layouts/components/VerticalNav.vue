...
const navItems = props.navItems.map(item => {
  ...
  if (hasRole('Staff Member')) {
    if (item.title === 'Account Settings') {
      item.children = item.children.filter(child => {
        return child.title === 'Edit Profile' || child.title === 'Password Change'
      })
    }

    if (item.title === 'Manage Users') {
      item.children = item.children.filter(child => child.title === 'Agent List')
    }
  }

  if (hasRole('Travel Agent')) {
    if (item.title === 'Traveler') {
      item.children = item.children.filter(child => child.title === 'Traveler List')
    }
  }
  ...
  } else if (hasRole('Travel Agent')) {
    return item.title === 'Dashboard' ||
      item.title === 'Booking' ||
      item.title === 'Traveler List' ||
      item.title === 'Agent Payment' ||
      item.title === 'Account Settings'
  } else {
    return item.title !== 'Agent Payment' &&
      item.title !== 'Traveler List'
  }
})
