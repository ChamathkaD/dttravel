...
const confirmRemoveOption = index => {
...
const confirmRemove = () => {
  if (selectedOptionIndex.value !== null) {
    form.itineraries.splice(selectedOptionIndex.value, 1)

    optionCounter.value = form.itineraries.length
    activeTab.value = Math.max(0, optionCounter.value - 1)
  }
...
const form = useForm({
  ...
  itineraries: [
    { day: 1, title: null, description: null, accommodation: null, food: null, location: null },
  ],
...
const handlePublishPackage = () => {
  itineraryForm.value?.validate().then(valid => {
    if (valid.valid) {
      ...
      form.post(route('travel-packages.store'), {
        preserveState: true,
        onSuccess: () => {
...
const handleValueAddFormSubmit = () => {
  ...
  valueAddForm.post(route('travel-packages.value-add.store'), {
    ...
    onSuccess: () => {
      ...
      Notify.success('Value Add Added!')
...
<template>
  <Head title="Add new travel package"/>
  <Breadcrumbs/>
  <div>
  ...
  <VRow>
    <VCol md="8">
      ...
      <VCard class="mb-6">
        <VCardItem>
          <template #title>
            Add 5 Images <span class="text-error text-sm">( 1 image is required)</span> *
          </template>
        </VCardItem>
                ...
                <div class="pe-3">
                  ...
                  >
                    <VTab
                      ...
                      border
                      elevation="0"
                      class="my-1 text-grey-500"
                      selected-class="bg-grey-100 border-0"
                    >
                ...
                >
                  ...
                  >
                    <div class="d-flex flex-column ps-3">
                      <VCol cols="12">
                        <span class="mb-1">Title of the Day *</span>
                        <AppTextField
                          v-model="item.title"
                        ...
                        <p
                          v-if="form.errors['itineraries.' + index + '.title']"
                          class="text-sm text-error mt-1"
                        >
                          {{ form.errors["itineraries." + index + ".title"] }}
                        </p>
                      </VCol>
                      ...
                      <VCol>
                        ...
                        <p
                          v-if="form.errors['itineraries.' + index + '.description']"
                          class="text-sm text-error mt-1"
                        >
                          {{ form.errors["itineraries." + index + ".description"] }}
                        </p>
                      </VCol>
                      ...
                      <VCol cols="12">
                        ...
                        <p
                          v-if="form.errors['itineraries.' + index + '.accommodation']"
                          class="text-sm text-error mt-1"
                        >
                          {{ form.errors["itineraries." + index + ".accommodation"] }}
                        </p>
                      </VCol>
                      ...
                      <VCol cols="12">
                        ...
                        <p
                          v-if="form.errors['itineraries.' + index + '.food']"
                          class="text-sm text-error mt-1"
                        >
                          {{ form.errors["itineraries." + index + ".food"] }}
                        </p>
                      </VCol>
                      ...
                      <VCol cols="12">
                        ...
                        <p
                          v-if="form.errors['itineraries.' + index + '.location']"
                          class="text-sm text-error mt-1"
                        >
                          {{ form.errors["itineraries." + index + ".location"] }}
                        </p>
                      </VCol>
                    </div>

                    <VCol :class="index === 0 ? 'd-flex justify-end' : 'd-flex justify-space-between'">
                      <VBtn
                        class="mt-6"
                        variant="text"
  ...
  >
    ...
    <!-- Dialog Content -->
    <VCard title="Are you sure delete this day?">
      <VCardText>
        The the itinerary couldn’t be recover after the deleted. make sure the before delete
      </VCardText>
