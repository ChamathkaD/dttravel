...
const adultPrice = computed(() => {
  return props.booking && props.booking.total_adults_price ? props.booking.total_adults_price : 0.00
})

const childPrice = computed(() => {
  return props.booking && props.booking.total_child_price ? props.booking.total_child_price : 0.00
})

const travelPackage = [
  {
    packageName: props.booking && props.booking.package ? props.booking.package.name : '',
    subtitle: props.booking && props.booking.package ? 'ID: ' + props.booking.package.id : '',
    adultPrice: 'Adults - $' + adultPrice.value,
    childPrice: 'Child - $' + childPrice.value,
    adultPax: props.booking ? (props.booking.adult_count ?? '-') : '-',
    childPax: props.booking ? (props.booking.child_count ?? '-') : '-',
    adultTotal: props.booking ? (props.booking.total_adults_price ?? '-') : '-',
    childTotal: props.booking ? (props.booking.total_child_price ?? '-') : '-',
  },
]
...
const goToBack = () => {
  ...
  const url = "https://dttravelssrilanka.com/bookings/" + bookingId
...
<template>
  <section>
    <VRow class="d-flex justify-center pa-10">
      <VCol
        cols="12"
        md="9"
